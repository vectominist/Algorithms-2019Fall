...
class SortTool {
    ...
    private:
        inline void QuickSortSubVector(vector<int>&, int, int); // quick sort subvector
        inline int  Partition(vector<int>&, int, int); // partition the subvector
        inline void MergeSortSubVector(vector<int>&, int*, int, int); // merge sort subvector
        inline void Merge(vector<int>&, int*, int, int, int); // merge two sorted subvector
        inline void MaxHeapify(vector<int>&, int); // make tree with given root be a max-heap 
